 ...
 * Description: This is a multi-threaded program to count the number of ASCII
 *              characters in a text file provided by the user.
 *              It uses an one-dimentional array to store the counts and uses
 *              Synchronization to assure consistency of data
 ******************************************************/
...
void* asciiCounter(void *info) {
    ...
    while (start != (end + 1)) {
        ...
        if (pthread_mutex_lock(&mutex) != 0 ) {
            fprintf(stderr, "acquire mutex failed");
        }
        ...
        if (pthread_mutex_unlock(&mutex) != 0 ) {
            fprintf(stderr, "release mutex failed");
        }
    }
